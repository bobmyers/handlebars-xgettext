...
function parse(files, options, callback) {
  ...
  options.keyword = options.keyword.reduce(function(result, keywordspec) {
    ...
    var ret = result[key] = {positional: {}, named: {}};

    argnums.forEach(function(argnum) {
      var match;

      if ((match = argnum.match(/(\d)c/))) {
        ret.positional.context = +match[1];
      } else {
        if (isNaN(argnum)) { //attribute name
          ret.named[argnum] = true;
        } else { //number
          ret.positional[first++ ? "plural" : "singular"] = +argnum;
        }
      }
    });
  ...
  function parseFiles (files) {
    var parser = new Parser(options.keyword, options.comment),
      strings,
      template,
      relativePath;
    ...
    files.forEach(function (file) {
      ...
      for (var msgid in strings) {
        if (strings.hasOwnProperty(msgid)) {
          ...
          // add extracted comments, if any
          if (strings[msgid].comment) {
            context[msgid].comments.extracted = strings[msgid].comment;
          }

          // add file/line number information, if requested
